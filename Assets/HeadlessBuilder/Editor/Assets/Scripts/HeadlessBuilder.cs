...
#if UNITY_2021_2_OR_NEWER
            if (!symbolArray.Contains("UNITY_SERVER"))
            {
...
#if UNITY_2021_2_OR_NEWER
            EditorUserBuildSettings.SwitchActiveBuildTarget(headlessSettings.valueDedicated ? NamedBuildTarget.Server : NamedBuildTarget.Standalone, buildTarget);
#endif
    ...
    static bool PerformBuild()
    {
        ...
        try
        {
            if (!cloudBuild && !debugBuild)
            {
                ...
                buildOptions = (BuildOptions)HeadlessEditorCallbacks.InvokeCallbacks("HeadlessBuildOptions", buildOptions);
                buildPlayerOptions.options = buildOptions;

                buildPlayerOptions = (BuildPlayerOptions)HeadlessEditorCallbacks.InvokeCallbacks("HeadlessBuildPlayerOptions", buildPlayerOptions);
